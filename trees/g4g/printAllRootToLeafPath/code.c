...
void printAllRootToLeafPath(struct tree_node *root, int *path, int pathLength) {
	...
	printAllRootToLeafPath(root->left,path,pathLength);
	printAllRootToLeafPath(root->right,path,pathLength);
...
int main() {
	...
	printAllRootToLeafPath(tree->root,path,0);
	return 0;
}
